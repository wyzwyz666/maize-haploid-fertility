...
def fit_one_epoch(backbone, model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    if local_rank == 0:
        ...
        print('Acc_Val: %.3f ' % (val_accuracy / epoch_step_val))
        ...
        #     torch.save(model.state_dict(), os.path.join(save_dir,f"ep{epoch + 1}-train_loss{total_loss / epoch_step:.3f}-val_loss{val_loss / epoch_step_val:.3f}-{backbone}.pth"))
        if (epoch + 1) % save_period == 0 or epoch + 1 == Epoch:
            torch.save(model.state_dict(), os.path.join(save_dir,f"ep{epoch + 1}-train_acc{total_accuracy / epoch_step:.1f}-val_acc{val_accuracy / epoch_step_val:.1f}-{backbone}.pth"))
